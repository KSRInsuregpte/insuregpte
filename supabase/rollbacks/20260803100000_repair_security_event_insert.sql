-- Safe rollback for the security-event insert repair.
--
-- This version relies on the existing updated_at table default for new rows.
-- It intentionally does not restore the malformed ten-column/nine-expression
-- statement because doing so would make all safety-event ingestion fail again.
...
BEGIN
    IF to_regclass('public.security_events') IS NULL
       OR to_regprocedure(
        ...
        RAISE EXCEPTION
            'The safety-event helper required by this rollback is missing';
    END IF;
END;
$guard$;
