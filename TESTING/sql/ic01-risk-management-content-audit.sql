-- Read-only IC01 Risk Management content audit.
...
ic01_modules as (
  ...
  join ic01_subject as subject_record
...
audit_rows as (
  ...
  from ic01_modules as module_record
  ...
  from ic01_chapters as chapter_record
  join ic01_modules as module_record
  ...
  from ic01_topics as topic_record
  join ic01_modules as module_record
    on module_record.id = topic_record.module_id
  join ic01_chapters as chapter_record
    on chapter_record.id = topic_record.chapter_id

  union all

  select
    '05_resource_types'::text,
    resource_type_record.code::text,
    pg_catalog.jsonb_build_object(
      'id', resource_type_record.id,
      'name', resource_type_record.name,
      'description', resource_type_record.description,
      'display_order', resource_type_record.display_order,
      'is_active', resource_type_record.is_active
    )
  from public.learning_resource_types as resource_type_record
  where resource_type_record.id in (
    select resource_record.resource_type_id
  ...
  join ic01_topics as topic_record
